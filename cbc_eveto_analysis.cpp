...
int eveto::cbc_eveto_analysis(
	...
	TFile *_file0=TFile::Open("cbc_eveto.root"); //this should already be open, via...where is it opened? Keep this line for now. 
	
	TTree* tree1 = (TTree*)_file0->Get("safe_channel_list");

	for (Long64_t i = 0; i < channel->GetEntries(); i++)
	{

	channel->GetEntry(i);

	snprintf( tree_name, 256, "VETOTRIGS:%s", channel_name );
	TTree* tree = (TTree*)_file0->Get(tree_name);	

	std::cerr << "successfully read in VETOTRIGS" << std::endl;

	//TTree* tree = (TTree*)_file0->Get("ntuple"); //or whatever the ntuple is *actually* called: In this case, loop over all of the tree names->  figure out labeling scheme from original eveto code. Use pointer within the plotting code to access each tree.

	//Import aux trigger tree, currently in -same- directory as post-eveto.C, rename tree,tree1 to more appropriate names--> this isn't necessary because all of the data is in one tree. Commenting out for now.
